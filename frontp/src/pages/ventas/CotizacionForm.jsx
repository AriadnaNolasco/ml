...
const CotizacionForm = ({ cotizacion, onSuccess, onCancel }) => {
  ...
  // ✅ NUEVO: Estado para tipo de cambio
  ...
  const [totales, setTotales] = useState({
    ...
    igv: 0,
    total: 0,
    importe_dolares: 0, // ✅ Solo conversión a dólares (cuando moneda=USD)
  });
  ...
  useEffect(() => {
    ...
    fetchTipoCambioDia(); // ✅ NUEVO: Cargar tipo de cambio del día
  ...
  useEffect(() => {
    calcularTotales();
  }, [productos, igvSeleccionado, tipoCambioActual]); // ✅ ACTUALIZADO: Agregar tipoCambioActual

  // ====================================================================
  // FUNCIONES DE CÁLCULO DE DESCUENTOS - ACTUALIZADAS
  // ====================================================================

  // ✅ NUEVA FUNCIÓN: Cargar tipo de cambio del día
  const fetchTipoCambioDia = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/ventas/tipo-cambio/actual`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      // ✅ Acceder correctamente a la estructura de respuesta
      if (response.data.success && response.data.data) {
        console.log("💱 Tipo de cambio cargado:", response.data.data);
        setTipoCambioActual(response.data.data);

        // Si no es edición y la moneda es USD, pre-cargar el tipo de cambio
        if (!cotizacion) {
          const monedaActual = form.getFieldValue("moneda_id");
          if (monedaActual === 2) {
            // 2 = USD (Dólares)
            form.setFieldsValue({
              tipo_cambio_id: response.data.data.id,
              tipo_cambio: response.data.data.venta,
            });
          }
        }
      } else {
        console.warn("No hay tipo de cambio disponible");
      }
    } catch (error) {
  ...
  const handleMonedaChange = (monedaId) => {
    setMonedaSeleccionada(monedaId);

    // Si es USD (Dólares) y hay tipo de cambio disponible, cargarlo
    if (monedaId === 2 && tipoCambioActual && tipoCambioActual.id) {
      form.setFieldsValue({
        tipo_cambio_id: tipoCambioActual.id,
        tipo_cambio: tipoCambioActual.venta,
      });
    } else {
      // Si es PEN (Soles), limpiar y deshabilitar tipo de cambio
      form.setFieldsValue({
        tipo_cambio_id: null,
        tipo_cambio: null,
      });
  ...
  const fetchDatosFormulario = async () => {
    try {
      ...
      const almacenesResponse = await axios.get(
        `${import.meta.env.VITE_API_URL}/almacen/almacenes`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const almacenesData = Array.isArray(almacenesResponse.data) ? almacenesResponse.data : [];
      setAlmacenes(almacenesData);

      // ✅ NUEVO: Establecer almacén por defecto (Almacén Central - código 110)
      if (!cotizacion && almacenesData.length > 0) {
        const almacenCentral = almacenesData.find(a => a.codigo === '110') || almacenesData[0];
        console.log(`📦 Almacén por defecto: ${almacenCentral.nombre} (${almacenCentral.codigo})`);
      }
  ...
  const cargarDatosCotizacion = (datoCotizacion = null) => {
    ...
    if (cotizacionData) {
      ...
      form.setFieldsValue({
        ...
        tipo_cambio_id: cotizacionData.tipo_cambio_id || null, // ✅ NUEVO
        tipo_cambio: cotizacionData.tipo_cambio || 0, // ✅ NUEVO
      ...
      // ✅ NUEVO: Establecer moneda seleccionada
      if (cotizacionData.moneda_id) {
        setMonedaSeleccionada(cotizacionData.moneda_id);
      }
  ...
  const seleccionarProducto = (value, option) => {
    ...
    const nuevoProductoBase = {
      ...
      // ✅ ACTUALIZADO: Usar stock disponible real considerando reservas
      stock_disponible: producto.stock_disponible_real || producto.stock_actual || 0,
      stock_actual: producto.stock_actual || 0, // Mantener stock actual por referencia
      stock_reservado_total: producto.stock_reservado_total || 0, // Total reservado
  ...
  const calcularTotales = () => {
    ...
    // ✅ CORREGIDO: Los precios SIEMPRE están en SOLES
    // Si moneda = USD, convertir total en soles a dólares
    let importe_dolares = 0;
    const tipoCambio = parseFloat(form.getFieldValue("tipo_cambio")) || 0;
    const monedaId = form.getFieldValue("moneda_id");

    if (monedaId === 2 && tipoCambio > 0) {
      // Si moneda es USD: dividir total en soles entre tipo de cambio
      // Ejemplo: S/ 1779.70 ÷ 3.2500 = $547.60
      importe_dolares = total / tipoCambio;
    }
    // Si moneda es PEN, NO hacer conversión (importe_dolares = 0)
    ...
    setTotales({
      ...
      importe_dolares: parseFloat(importe_dolares.toFixed(2)),
    });
  };
  ...
  const guardarCotizacion = async (values) => {
    ...
    try {
      ...
      const cotizacionCompleta = {
        ...
        tipo_cambio_id: values.tipo_cambio_id || null, // ✅ NUEVO
        tipo_cambio: values.tipo_cambio || null, // ✅ NUEVO
      ...
      // ✅ CORREGIDO: Mostrar conversión solo si moneda=USD
      const monedaId = values.moneda_id;
      const tipoCambio = parseFloat(values.tipo_cambio) || 0;

      // Solo mostrar conversión si moneda es USD y hay tipo de cambio
      if (monedaId === 2 && tipoCambio > 0) {
        const total = parseFloat(response.data.cotizacion?.total || 0);
        const totalUsd = total / tipoCambio;
        message.info({
          content: `Equivalente en dólares: $${totalUsd.toFixed(
            2
    ...
    >
      ...
      <Card title="Datos de la Cotización" style={{ marginBottom: 16 }}>
        <Row gutter={16}>
          ...
          <Col xs={24} md={6}>
            ...
            >
              <Select
                ...
                onChange={handleMonedaChange} // ✅ NUEVO: Manejar cambio de moneda
              >
                {monedas.map((moneda) => (
          ...
          {/* ✅ NUEVO: Campos de Tipo de Cambio */}
          <Col xs={24} md={6}>
            <Form.Item label="Tipo de Cambio (ID)" name="tipo_cambio_id">
              <InputNumber
                style={{ width: "100%" }}
                placeholder="Auto"
                disabled
          ...
          <Col xs={24} md={6}>
            <Form.Item
              label="T.C. Venta (S/)"
              name="tipo_cambio"
              tooltip="Tipo de cambio para conversión. Solo se usa con moneda USD."
            >
              <InputNumber
                style={{ width: "100%" }}
                placeholder="0.0000"
                ...
                precision={4}
                step={0.0001}
                disabled={monedaSeleccionada !== 2} // ✅ Solo editable si es USD
                onChange={() => calcularTotales()} // Recalcular al cambiar
              />
            </Form.Item>
          </Col>
          ...
          {/* ✅ NUEVO: Información del Tipo de Cambio */}
          {tipoCambioActual &&
            tipoCambioActual.id &&
            form.getFieldValue("moneda_id") === 2 && (
              <Col xs={24}>
                <Alert
                  message={
                    <Space
                    ...
                    >
                      <div>
                        <strong>💱 Tipo de Cambio del Día:</strong>{" "}
                        {tipoCambioActual.fecha}
                      </div>
                      <div>
                        Compra: S/{" "}
                        {parseFloat(tipoCambioActual.compra).toFixed(4)} |
                        Venta: S/{" "}
                        {parseFloat(tipoCambioActual.venta).toFixed(4)}
                      </div>
                      <div style={{ fontSize: "12px" }}>
                        ℹ️ Se usará para calcular el equivalente en dólares
                        automáticamente
                      </div>
                    </Space>
                  }
        ...
        <Row gutter={16}>
          <Col xs={24} md={12}>
            <Space direction="vertical" style={{ width: "100%" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <strong>
                  {"S/"} {totales.importe_bruto.toFixed(2)}
                </strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <strong>
                  {"S/"} {totales.valor_venta.toFixed(2)}
                </strong>
              </div>
            </Space>
          </Col>
          <Col xs={24} md={12}>
            <Space direction="vertical" style={{ width: "100%" }}>
              ...
              {/* ✅ CORREGIDO: Mostrar conversión a DÓLARES solo si moneda=USD */}
              {form.getFieldValue("moneda_id") === 2 &&
                totales.importe_dolares > 0 && (
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                  ...
                  >
                    <span>
                      ...
                      {form.getFieldValue("tipo_cambio") && (
                        <span
                          style={{
                            fontSize: "12px",
                            color: "#8c8c8c",
                    ...
                    <strong style={{ color: "#52c41a" }}>
                      $ {totales.importe_dolares.toFixed(2)}
                    </strong>
                  </div>
                )}
            </Space>
      ...
      <Modal
        ...
        cancelText="Cancelar"
        okButtonProps={{
          disabled: !nuevoProducto.producto_codigo
        }}
        width={700}
      >
          ...
          {nuevoProducto.producto_codigo && (
            <>
              ...
              {(nuevoProducto.descuento_1 > 0 ||
                ...
                <Alert
                  message={`Descuento calculado: ${
                    nuevoProducto.descuento_1
                  }% + ${nuevoProducto.descuento_2}% = ${"S/"} ${
                    nuevoProducto.descuento_monto
                  }`}
                  type="info"
                  showIcon
                  size="small"
              ...
              <div>
                ...
                <Input
                  value={(() => {
                    const precios = calcularPreciosProducto(nuevoProducto);
                    const simbolo = "S/";
                    return `${simbolo} ${precios.valor_venta.toFixed(
                      2
                    )} + IGV ${simbolo} ${precios.igv.toFixed(
                      2
                    )} = ${simbolo} ${precios.precio_total.toFixed(2)}`;
                  })()}
                  readOnly
