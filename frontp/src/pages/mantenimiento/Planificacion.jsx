...
const Planificacion = () => {
  ...
  const dateCellRender = (value) => {
    ...
    // FIX: backend devuelve "fecha", no "fecha_programada"
    const eventos = calendario.filter(ev => ev.fecha === fecha);
  ...
  const columns = [
    { 
      title: 'Equipo', 
      dataIndex: 'equipo_nombre'   // FIX ✔ 
    },
    { 
      title: 'Frecuencia',
      render: (_, row) => `${row.frecuencia_valor} ${row.frecuencia_tipo.toLowerCase()}`
    },
    { 
      title: 'Próxima Fecha', 
      dataIndex: 'proxima_fecha'   // FIX ✔ 
    },
    {
      title: 'Estado',
      dataIndex: 'estado',
    ...
    },
    {
      title: 'Acciones',
      render: (_, row) => (
        <Button danger size="small" onClick={() => eliminar(row.id)}>
          Eliminar
        </Button>
      ),
    }
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <Row gutter={24}>
        ...
        <Col span={8}>
          <Card title="Resumen">
            <p><Text strong>Planes Activos:</Text> {resumen.planes_activos}</p>
            <p><Text strong>Próximos 7 días:</Text> {resumen.proximos_7_dias}</p>
