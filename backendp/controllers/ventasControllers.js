...
const { crearPedidoDesdeCotizacion } = require("./pedidosController");
  ...
  try {
    ...
    let query = `
      SELECT
        ...
        p.descripcion,
        p.stock_actual,
        -- ✅ NUEVO: Stock disponible real considerando reservas
        almacen.calcular_stock_disponible(p.id_producto) as stock_disponible_real,
        -- ✅ NUEVO: Total reservado del producto
        (
          SELECT COALESCE(SUM(r.cantidad_reservada), 0)
          FROM almacen.reservas_stock r
          WHERE r.producto_id = p.id_producto
            AND r.estado = 'ACTIVA'
        ) as stock_reservado_total,
        p.precio_venta,
        p.precio_unitario,
        p.precio_total,
...
// ✅ FUNCIÓN 1: Obtener tipo de cambio del día
const obtenerTipoCambioDia = async (req, res) => {
  try {
    const query = `
      SELECT 
        tc.id,
    ...
    // Si no hay tipo de cambio del día, buscar el más reciente
    if (result.rows.length === 0) {
      const queryReciente = `
        SELECT 
          tc.id,
      ...
      result = await pool.query(queryReciente);
    }
...
// ✅ FUNCIÓN 2: Calcular importe en dólares (LÓGICA CORREGIDA)
const calcularImporteDolares = (total, monedaCodigo, tipoCambioVenta) => {
  if (monedaCodigo === 'USD' && tipoCambioVenta > 0) {
    // Si cotización está en DÓLARES, el total está en SOLES
    // Convertir de SOLES a DÓLARES: dividir entre tipo de cambio
    return parseFloat(total) / parseFloat(tipoCambioVenta);
  } else if (monedaCodigo === 'PEN') {
    // Si está en SOLES, NO hay conversión
    return 0;
  }
  return 0;
};

// ✅ CREAR COTIZACIÓN COMPLETA - ACTUALIZADA
const crearCotizacionCompleta = async (req, res) => {
  try {
    ...
    const {
      ...
      id_cliente,
      moneda_id,
      tipo_cambio_id,        // ✅ NUEVO: ID del tipo de cambio
      tipo_cambio,           // ✅ NUEVO: Monto del tipo de cambio
      forma_pago_id,
      comentario,
      prioridad,
      reparacion,
    ...
    // ✅ NUEVO: Si se proporciona tipo de cambio, validar que ambos campos estén presentes
    if ((tipo_cambio_id && !tipo_cambio) || (!tipo_cambio_id && tipo_cambio)) {
      await pool.query("ROLLBACK");
      return res.status(400).json({
        error: "Si proporciona tipo_cambio_id, debe proporcionar tipo_cambio y viceversa",
      });
    }
    ...
    for (const detalle of detalles) {
      ...
      // Usar precio_original si está disponible, sino usar precio_unitario
    ...
    // ✅ NUEVO: Calcular importe en dólares si se proporcionó tipo de cambio
    let importe_dolares = 0.00;
    if (tipo_cambio && tipo_cambio > 0) {
      importe_dolares = total / tipo_cambio;
    }

    // ✅ ACTUALIZADO: Insertar cabecera de cotización con campos nuevos
    ...
    const cotizacionValues = [
      ...
      moneda_id,
      tipo_cambio_id || null,      // ✅ NUEVO
      tipo_cambio || null,             // ✅ NUEVO
      forma_pago_id,
      reparacion || false,
      prioridad || "NORMAL",
      comentario || "",
    ...
    // ✅ ACTUALIZADO: Insertar los detalles de la cotización con importe_dolares
    for (let i = 0; i < detalles.length; i++) {
      ...
      const {
        ...
        descuento_2 = 0,
        descuento_monto = 0,
        // stock_disponible ya no se usa del frontend, se calcula en tiempo real
        fecha_entrega,
        prioridad: prioridadItem,
      } = detalle;

      // Usar precio_original como base para cálculos
      const precioBase = precio_original || precio_unitario;
      const precioBrutoItem = cantidad * precioBase;
      ...
      // ✅ NUEVO: Calcular importe en dólares del detalle
      let importe_dolares_item = 0.00;
      if (tipo_cambio && tipo_cambio > 0) {
        importe_dolares_item = precio_total_item / tipo_cambio;
      }

      // ✅ NUEVO: Obtener stock disponible REAL (considerando reservas)
      let stock_disponible_real = 0;
      if (producto_id) {
        const stockQuery = `
          SELECT almacen.calcular_stock_disponible($1) as stock_disponible
        `;
        const stockResult = await pool.query(stockQuery, [producto_id]);
        stock_disponible_real = stockResult.rows[0]?.stock_disponible || 0;
      }

      // Mantener precio_unitario igual al precio_original
      ...
      const detalleValues = [
        ...
        stock_disponible_real, // ✅ USA STOCK DISPONIBLE REAL
        cantidad,
...
// ✅ FUNCIÓN AUXILIAR ACTUALIZADA - obtenerCotizacionCompletaPorId
const obtenerCotizacionCompletaPorId = async (id_cotizacion) => {
  const queryCabecera = `
    SELECT 
      ...
      COALESCE(ifc.descuento_1, 0) as descuento_1,
      COALESCE(ifc.descuento_2, 0) as descuento_2,
      -- Asegurar que siempre traemos importe_dolares
      COALESCE(c.importe_dolares, 0) as importe_dolares,
    ...
    LEFT JOIN contabilidad.tipo_cambio tc ON c.tipo_cambio_id = tc.id
    LEFT JOIN public.usuarios u_created ON c.created_by = u_created.id
    LEFT JOIN public.usuarios u_updated ON c.updated_by = u_updated.id
    LEFT JOIN ventas.info_financiera_clientes ifc ON c.id_cliente = ifc.id_cliente
    LEFT JOIN public.documentos d ON c.id_documento = d.id_documento
  ...
  const queryDetalle = `
    SELECT
      ...
      -- ✅ NUEVO: Información de stock considerando reservas
      COALESCE(p.stock_actual, 0) as stock_actual,
      almacen.calcular_stock_disponible(p.id_producto) as stock_disponible_real,

      -- ✅ NUEVO: Calcular stock reservado total del producto
      (
        SELECT COALESCE(SUM(r.cantidad_reservada), 0)
        FROM almacen.reservas_stock r
        WHERE r.producto_id = p.id_producto
      ...
      -- Asegurar todos los campos de precios
      COALESCE(dc.precio_original, dc.precio_unitario) as precio_unitario_corregido,
      -- Calcular campos si no existen
      COALESCE(dc.precio_bruto, dc.cantidad * dc.precio_unitario) as precio_bruto_calculado,
      COALESCE(dc.valor_venta, dc.cantidad * dc.precio_unitario *
        (1 - COALESCE(dc.descuento_1, 0)/100) *
        (1 - COALESCE(dc.descuento_2, 0)/100)) as valor_venta_calculado,
      -- Calcular IGV si no existe
      COALESCE(dc.igv,
        (dc.cantidad * dc.precio_unitario *
         (1 - COALESCE(dc.descuento_1, 0)/100) *
         (1 - COALESCE(dc.descuento_2, 0)/100)) * 0.18) as igv_calculado,
      -- Calcular precio total si no existe
      COALESCE(dc.precio_total,
        (dc.cantidad * dc.precio_unitario *
         (1 - COALESCE(dc.descuento_1, 0)/100) *
         (1 - COALESCE(dc.descuento_2, 0)/100)) * 1.18) as precio_total_calculado
  ...
  try {
    ...
    // Procesar detalles para asegurar datos consistentes
    const detallesCorregidos = resultDetalle.rows.map((detalle) => {
      // Calcular descuento monto si no existe
      let descuentoMonto = detalle.descuento_monto;
      if (!descuentoMonto || descuentoMonto === 0) {
        const precioBruto = detalle.cantidad * detalle.precio_unitario;
        const desc1 = (precioBruto * (detalle.descuento_1 || 0)) / 100;
        const precioDespuesDesc1 = precioBruto - desc1;
        const desc2 = (precioDespuesDesc1 * (detalle.descuento_2 || 0)) / 100;
        descuentoMonto = desc1 + desc2;
      }

      // Calcular IGV por ítem si no existe
      let igvItem = detalle.igv;
      if (!igvItem || igvItem === 0) {
        const valorVenta = detalle.valor_venta || 
          (detalle.cantidad * detalle.precio_unitario * 
           (1 - (detalle.descuento_1 || 0)/100) * 
           (1 - (detalle.descuento_2 || 0)/100));
      ...
      return {
        ...
        igv_item: igvItem,
        precio_unitario: detalle.precio_unitario_corregido || detalle.precio_unitario,
        precio_bruto: detalle.precio_bruto || detalle.precio_bruto_calculado,
        valor_venta: detalle.valor_venta || detalle.valor_venta_calculado,
        igv: detalle.igv || detalle.igv_calculado,
        precio_total: detalle.precio_total || detalle.precio_total_calculado
      };
    });
...
// ✅ ACTUALIZAR COTIZACIÓN COMPLETA - ACTUALIZADA
const actualizarCotizacionCompleta = async (req, res) => {
  try {
    ...
    // ✅ ACTUALIZADO: Actualizar cabecera con campos nuevos
    const updateCotizacionQuery = `
      ...
      SET 
        ...
        reparacion = COALESCE($5, reparacion),
        prioridad = COALESCE($6, prioridad),
        comentario = COALESCE($7, comentario),
        igv_id = COALESCE($8, igv_id),
        importe_bruto = $9,
        monto_descuento = $10,
        valor_venta = $11,
        igv = $12,
    ...
    // ✅ ACTUALIZADO: Insertar los nuevos detalles con importe_dolares
    for (let i = 0; i < detalles.length; i++) {
      const detalle = detalles[i];
      const {
        producto_id,
...
// ✅ OBTENER COTIZACIÓN POR ID - YA USA obtenerCotizacionCompletaPorId
  ...
  try {
    ...
    const query = `
      SELECT
        ...
        -- ✅ NUEVO: Stock reservado total del producto
        (
          SELECT COALESCE(SUM(r.cantidad_reservada), 0)
          FROM almacen.reservas_stock r
          WHERE r.producto_id = p.id_producto
        ...
        -- Calcular el IGV para cada item
        (dc.valor_venta * 0.18) as igv_item
      FROM ventas.detalle_cotizacion dc
      LEFT JOIN almacen.productos p ON dc.producto_id = p.id_producto
      LEFT JOIN public.unidades_medida um ON p.id_unidad = um.id_unidades
      WHERE dc.cotizacion_id = $1
...
// Aprobar cotización y generar pedido automáticamente
// VERSIÓN CON pool.query() - SIN pool.connect()
const aprobarCotizacion = async (req, res) => {
  try {
    ...
    try {
      // Verificar primero si ya existe un pedido para esta cotización
      const pedidoExistenteQuery = `
        SELECT id_pedido FROM ventas.pedidos_cliente 
        WHERE id_cotizacion = $1
      `;
      const pedidoExistenteResult = await pool.query(pedidoExistenteQuery, [id]);

      if (pedidoExistenteResult.rows.length === 0) {
        // No existe pedido, crear uno nuevo
        pedidoGenerado = await crearPedidoDesdeCotizacion(id, usuario_id, {});
        mensajePedido = `Pedido #${pedidoGenerado.numero} generado automáticamente`;
      } else {
        mensajePedido = "Ya existe un pedido para esta cotización";
      }
    } catch (pedidoError) {
      console.error("Error al generar pedido automático:", pedidoError);
      // No fallar la aprobación si falla la generación del pedido
      mensajePedido = `Advertencia: ${pedidoError.message}`;
    }
    ...
    res.json({
      ...
      pedido_info: mensajePedido,
      cotizacion: cotizacionCompleta,
      pedido: pedidoGenerado, // Incluir datos del pedido generado
    });
  } catch (error) {
  ...
  try {
    ...
    const query = `
      ...
      SET
        ...
        updated_by = $1,
        updated_at = CURRENT_TIMESTAMP
      WHERE id_cotizacion = $2 AND estado != 'ANULADO'
      RETURNING *
    `;
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        error: "Cotización no encontrada o ya está anulada",
      });
    }
...
// En ventasControllers.js, modifica la función generarPDFCotizacion:

// Función para generar PDF de cotización con importe en dólares
const generarPDFCotizacion = async (req, res) => {
  try {
    ...
    // Calcular importe en dólares si no está en los datos
    let importe_dolares = parseFloat(cotizacionCompleta.importe_dolares) || 0;

    if (importe_dolares === 0 && cotizacionCompleta.tipo_cambio && cotizacionCompleta.tipo_cambio > 0) {
      importe_dolares = parseFloat(cotizacionCompleta.total) / parseFloat(cotizacionCompleta.tipo_cambio);
    }
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        `Vendedor: ${cotizacionCompleta.vendedor_nombre ||
    ...
    // ===== TOTALES GENERALES CON IMPORTE EN DÓLARES =====

    // Panel de totales con diseño mejorado
    const totalesWidth = 280;
    const totalesX = pageWidth - margin - totalesWidth;

    doc
      .fillColor("#f8f9fa")
      .rect(totalesX, yPosition, totalesWidth, 140)
      .fill()
      .strokeColor(primaryColor)
      .lineWidth(1)
      .stroke();
    ...
    if (importe_dolares > 0 && (cotizacionCompleta.moneda_codigo === 'USD' || cotizacionCompleta.tipo_cambio > 0)) {
      // Línea separadora
      doc
        .strokeColor("#1890ff")
        .lineWidth(0.5)
        .moveTo(totalesX + 15, totalYStart + lineHeight * 5 + 8)
        .lineTo(totalesX + totalesWidth - 15, totalYStart + lineHeight * 5 + 8)
        .stroke();
      ...
      doc
        ...
        .fontSize(10)
        .font("Helvetica-Bold")
        .text("TOTAL EN DÓLARES:", totalesX + 15, totalYStart + lineHeight * 5 + 15, {
          width: labelWidth,
          align: "left",
        })
        .text(
          `US$ ${importe_dolares.toFixed(2)}`,
          valueX,
          totalYStart + lineHeight * 5 + 15,
          { width: valueWidth, align: "right" }
        );

      // Tipo de cambio
      if (cotizacionCompleta.tipo_cambio && cotizacionCompleta.tipo_cambio > 0) {
        doc
          .fillColor("#666")
          .fontSize(7)
          .font("Helvetica")
          .text(`Tipo de cambio: ${parseFloat(cotizacionCompleta.tipo_cambio).toFixed(4)}`,
            totalesX + 15, totalYStart + lineHeight * 5 + 30, {
            width: totalesWidth - 30,
            align: "center",
          });
      }
    }
    ...
    doc
      ...
      .fontSize(8)
      .font("Helvetica-Bold")
      .text("SON:", margin, yPosition + 70, { width: 40, align: "left" })
      .font("Helvetica")
      .text(
    ...
    // ===== INFORMACIÓN ADICIONAL CON IMPORTE EN DÓLARES =====
    ...
    doc
      ...
      .text(
        `Válida por 30 días | Estado: ${cotizacionCompleta.estado || "PENDIENTE"}`,
        margin,
        infoY + 12
      );

    // Agregar información de moneda y tipo de cambio si aplica
    if (importe_dolares > 0) {
      doc
        .text(
          `Moneda: ${cotizacionCompleta.moneda_nombre || 'Nuevos Soles'} | Tipo Cambio: ${parseFloat(cotizacionCompleta.tipo_cambio || 0).toFixed(4)}`,
          margin,
          infoY + 24
        )
        .text(
          `Equivalente en dólares: US$ ${importe_dolares.toFixed(2)}`,
          margin,
          infoY + 36
        );
    } else {
      doc
        .text(
          `Creado por: ${cotizacionCompleta.creado_por_nombre || "Sistema"} | Código: VE-R-0101`,
          margin,
          infoY + 24
        );
    }
...
const generarPDFCotizacionSimple = async (req, res) => {
  try {
    ...
    cotizacionCompleta.detalles.forEach((detalle, index) => {
      doc.text(
        `${index + 1}. ${detalle.descripcion_producto} - S/ ${detalle.precio_total
        }`,
        120,
        y
      );
