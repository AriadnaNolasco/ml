...
const contabilidadController = {
    ...
    crearFacturaProveedor: async (req, res) => {
        try {
            const {
                ...
                // ⚠️ NUEVO: DETALLE DE FACTURA
                items = []   // 👈 viene del front
            } = req.body;

            // Validaciones básicas
            if (!documento_id || !orden_compra_id || !proveedor_id || !tipo_compra) {
                console.log('❌ Faltan campos base:', { documento_id, orden_compra_id, proveedor_id, tipo_compra });
                return res.status(400).json({
                    success: false,
            ...
            if (subtotal == null || total == null || tipo_cambio == null || importe_soles == null) {
                console.log('❌ Validación montos comunes falló. Body recibido:', req.body);
                return res.status(400).json({
                    success: false,
            ...
            // ⚠️ Validar que existan items
            if (!Array.isArray(items) || items.length === 0) {
                console.log('❌ Items inválidos o vacíos. items:', items);
                return res.status(400).json({
                    success: false,
                    message: 'Debe enviar el detalle de la factura (items)'
                });
            }
            ...
            const { rows } = await db.query(query, values);
            const factura = rows[0];              // 👈 cabecera insertada
            ...
            const detalleQuery = `
                INSERT INTO contabilidad.facturas_proveedor_detalle (
                    factura_id,
                    orden_compra_id,
            ...
            for (const item of items) {
                // aquí asumo la forma en que vendrán los campos desde el front
                const {
                    orden_compra_detalle_id,
                    numitem,
                    producto_codigo,
                ...
                // validación mínima de cantidad_facturada
                if (!cantidad_facturada || cantidad_facturada <= 0) {
                    console.log('❌ Item con cantidad_facturada inválida:', item);
                    await db.query('ROLLBACK');
                    return res.status(400).json({
                        success: false,
                        message: 'Todos los items deben tener cantidad_facturada mayor a 0'
                    });
                }
                ...
                const detalleValues = [
                    ...
                    valor_venta || 0,
                    tipo_compra === 'LOCAL' ? (igv_item || 0) : null,
                    precio_total || 0
                ];

                await db.query(detalleQuery, detalleValues);
            }
        ...
        try {
            const query = `
                SELECT 
                    ...
                    fp.otros_gastos,
                    fp.importe_cif,
                    fp.importe_moneda_prov,
                    fp.importe_soles,
                    fp.banco_id,
                    fp.cuenta_bancaria,
                    fp.cuenta_interbancaria,
                    fp.swift,
                    fp.direccion_banco,
                    fp.guia_remision,
                    fp.detraccion,
                    fp.retencion,
                    fp.fecha_guia_remision,
                    fp.comentario,
                    fp.archivo_factura,
                    fp.estado,
                    fp.fecha_registro,
                    ...
                    fop.nombre AS forma_pago_nombre,
                    ...
                    COALESCE(
                        json_agg(
                            json_build_object(
                                'id', fpd.id,
                                'factura_id', fpd.factura_id,
                                'orden_compra_id', fpd.orden_compra_id,
                                'orden_compra_detalle_id', fpd.orden_compra_detalle_id,
                                'numitem', fpd.numitem,
                                'producto_codigo', fpd.producto_codigo,
                                'descripcion', fpd.descripcion,
                                'unidad_medida', fpd.unidad_medida,
                                'cantidad_solicitada', fpd.cantidad_solicitada,
                                'cantidad_recibida', fpd.cantidad_recibida,
                                'cantidad_facturada', fpd.cantidad_facturada,
                                'precio_unitario', fpd.precio_unitario,
                                'descuento_porcentaje', fpd.descuento_porcentaje,
                                'valor_venta', fpd.valor_venta,
                                'igv', fpd.igv,
                                'precio_total', fpd.precio_total,
                                'created_at', fpd.created_at
                            )
                        ) FILTER (WHERE fpd.id IS NOT NULL),
                        '[]'::json
                    ) AS items
                FROM contabilidad.facturas_proveedor fp
                LEFT JOIN public.documentos d 
                    ON fp.documento_id = d.id_documento
                LEFT JOIN compras.proveedores p 
                    ON fp.proveedor_id = p.id_prov
                LEFT JOIN compras.orden_compra oc 
                    ON fp.orden_compra_id = oc.id
                LEFT JOIN contabilidad.cod_moneda m 
                    ON fp.moneda_id = m.id_moneda
                LEFT JOIN contabilidad.formas_pago fop 
                    ON fp.forma_pago_id = fop.id
                LEFT JOIN public.incoterms i 
                    ON fp.incoterm_id = i.id
                LEFT JOIN public.medios_transporte mt 
                    ON fp.medio_transporte_id = mt.id
                LEFT JOIN public.cod_aduana a 
                    ON fp.aduana_id = a.id_aduana
                LEFT JOIN public.bancos b 
                    ON fp.banco_id = b.id_bancos
                LEFT JOIN contabilidad.facturas_proveedor_detalle fpd
                    ON fp.id = fpd.factura_id
                GROUP BY 
                    fp.id,
                    d.codigo, d.nombre,
                    p.razon_social, p.nro_documento,
                    oc.numero,
                    m.codigo, m.nombre,
                    fop.nombre,
                    i.codigo, i.nombre,
    ...
    // Función para obtener todas las facturas de proveedor con su detalle
    getFacturaProveedorById: async (req, res) => {
        try {
            const { id } = req.params; // 👈 id que viene de /:id/detalles
            ...
            console.error('Error al obtener factura de proveedor por id:', error);
            res.status(500).json({
                success: false,
    ...
    // Función para exportar factura de proveedor en PDF 
    exportFacturaProveedorPDF: async (req, res) => {
    try {
        const { id } = req.params;
        ...
        LEFT JOIN contabilidad.formas_pago fop ON fp.forma_pago_id = fop.id
        ...
        const { rows: facturaRows } = await db.query(facturaQuery, [id]);
        ...
        SELECT 
            ...
            igv,
            precio_total
        FROM contabilidad.facturas_proveedor_detalle
        WHERE factura_id = $1
        ORDER BY numitem
        `;
        ...
        res.setHeader(
        'Content-Disposition',
        `attachment; filename="factura-${factura.documento_codigo}-${factura.numero}.pdf"`
        );
        ...
        // =============== ENCABEZADO TIPO “REQUERIMIENTO” ==================
        // Bloque empresa izquierda (puedes cambiar los datos fijos por los tuyos)
        let y = 40;

        doc
        .font('Helvetica-Bold')
        .fontSize(11)
        .text('NOMBRE DE LA EMPRESA S.A.C.', 40, y);

        y += 13;
        doc
        .font('Helvetica')
        .fontSize(9)
        .text('RUC: 20101234567', 40, y);
        y += 11;
        doc.text('Dirección de la empresa', 40, y);
        y += 11;
        doc.text('Ciudad - País', 40, y);

        // Caja a la derecha: tipo de documento + número
        const boxX = 350;
        const boxY = 40;
        const boxW = 205;
        const boxH = 70;
        ...
        .font('Helvetica-Bold')
        .fontSize(12)
        .text('FACTURA PROVEEDOR', boxX, boxY + 10, {
            width: boxW,
            align: 'center'
        });
        ...
        .fontSize(9)
        .text(
            `${factura.documento_codigo || ''} N° ${factura.numero || ''}`,
            boxX,
            boxY + 30,
        ...
        // =============== DATOS DE LA FACTURA (similar “Datos del requerimiento”) ============
        y = 130;

        doc
        .font('Helvetica-Bold')
        .fontSize(10)
        .text('DATOS DE LA FACTURA', 40, y);
        ...
        // Se arma en dos columnas, tipo ficha como en la imagen
        const col1X = 40;
        const col2X = 300;
        ...
        doc
        .font('Helvetica-Bold')
        .text('Proveedor:', col1X, y);
        doc
        .font('Helvetica')
        .text(factura.proveedor_nombre || '', col1X + 70, y, { width: 220 });
        ...
        doc
        .font('Helvetica')
        .text(factura.proveedor_ruc || 'No especificado', col1X + 70, y);

        y += 12;
        doc
        .font('Helvetica-Bold')
        .text('Dirección:', col1X, y);
        doc
        .font('Helvetica')
        .text(factura.direccion || 'No especificada', col1X + 70, y, {
            width: 220
        });
        ...
        doc
        .font('Helvetica-Bold')
        .text('Tipo Compra:', col1X, y);
        doc
        .font('Helvetica')
        .text(factura.tipo_compra || '-', col1X + 70, y);
        ...
        doc
        .font('Helvetica-Bold')
        .text('Orden Compra:', col2X, y2);
        doc
        .font('Helvetica')
        .text(factura.orden_compra_numero || '-', col2X + 85, y2);

        y2 += 12;
        if (factura.tipo_compra === 'LOCAL') {
        doc
            .font('Helvetica-Bold')
            .text('Tipo Doc.:', col2X, y2);
        doc
            .font('Helvetica')
            .text(factura.tipo_doc || '-', col2X + 85, y2);
        ...
        doc
            .font('Helvetica')
            .text(factura.serie || '-', col2X + 85, y2);

        y2 += 12;
        doc
            .font('Helvetica-Bold')
            .text('N° Factura:', col2X, y2);
        doc
            .font('Helvetica')
            .text(factura.numero_fac || '-', col2X + 85, y2);
        } else {
        doc
            .font('Helvetica-Bold')
            .text('Invoice:', col2X, y2);
        doc
            .font('Helvetica')
            .text(factura.numero_invoice || '-', col2X + 85, y2);

        y2 += 12;
        doc
            .font('Helvetica-Bold')
            .text('F. Llegada:', col2X, y2);
        doc
            .font('Helvetica')
            .text(formatDate(factura.fecha_llegada) || '-', col2X + 85, y2);

        y2 += 12;
        doc
            .font('Helvetica-Bold')
            .text('Incoterm:', col2X, y2);
        doc
            .font('Helvetica')
            .text(factura.incoterm_nombre || '-', col2X + 85, y2);
        }

        const yAfterDatos = Math.max(y, y2) + 10;

        doc
        .moveTo(40, yAfterDatos)
        .lineTo(555, yAfterDatos)
        .stroke();

        // =============== DETALLE DE PRODUCTOS ==========================
        let yDet = yAfterDatos + 10;

        doc
        .font('Helvetica-Bold')
        .fontSize(10)
        .text('DETALLE DE PRODUCTOS', 40, yDet);
        ...
        // Encabezado tipo tabla, fondo gris clarito usando solo negro
        const headerY = yDet;
        doc
        .rect(40, headerY, 515, 16)
        .fillAndStroke('#E0E0E0', '#000000'); // gris claro
        doc.fillColor('#000000');

        doc
        .font('Helvetica-Bold')
        .fontSize(8)
        .text('Item', 45, headerY + 4, { width: 25 })
        .text('Código', 70, headerY + 4, { width: 60 })
        .text('Descripción', 130, headerY + 4, { width: 200 })
        .text('U.M.', 330, headerY + 4, { width: 30, align: 'center' })
        .text('Cant.', 360, headerY + 4, { width: 40, align: 'right' })
        .text('P.Unit.', 400, headerY + 4, { width: 50, align: 'right' })
        .text('V.Venta', 450, headerY + 4, { width: 50, align: 'right' });

        if (factura.tipo_compra === 'LOCAL') {
        doc.text('IGV', 500, headerY + 4, { width: 30, align: 'right' });
        doc.text('Total', 530, headerY + 4, { width: 25, align: 'right' });
        } else {
        doc.text('Total', 500, headerY + 4, { width: 55, align: 'right' });
        }
        ...
        if (yDet > 700) {
            ...
            .font('Helvetica-Bold')
            .fontSize(8)
            .text('Item', 45, hY + 4, { width: 25 })
            .text('Código', 70, hY + 4, { width: 60 })
            .text('Descripción', 130, hY + 4, { width: 200 })
            .text('U.M.', 330, hY + 4, { width: 30, align: 'center' })
            .text('Cant.', 360, hY + 4, { width: 40, align: 'right' })
            .text('P.Unit.', 400, hY + 4, { width: 50, align: 'right' })
            .text('V.Venta', 450, hY + 4, { width: 50, align: 'right' });

            if (factura.tipo_compra === 'LOCAL') {
            doc.text('IGV', 500, hY + 4, { width: 30, align: 'right' });
            doc.text('Total', 530, hY + 4, { width: 25, align: 'right' });
            } else {
            doc.text('Total', 500, hY + 4, { width: 55, align: 'right' });
            }
        ...
        doc
            .font('Helvetica')
            .fontSize(8)
            .text(item.numitem.toString(), 45, yDet, { width: 25 })
            .text(item.producto_codigo || '', 70, yDet, { width: 60 })
            .text(item.descripcion || '', 130, yDet, { width: 200 })
            .text(item.unidad_medida || '-', 330, yDet, {
            width: 30,
            align: 'center'
            })
            .text(formatCurrency(item.cantidad_facturada), 360, yDet, {
            width: 40,
            align: 'right'
            })
            .text(formatCurrency(item.precio_unitario), 400, yDet, {
            width: 50,
            align: 'right'
            })
            .text(formatCurrency(item.valor_venta), 450, yDet, {
            width: 50,
            align: 'right'
            });

        if (factura.tipo_compra === 'LOCAL') {
            doc.text(formatCurrency(item.igv || 0), 500, yDet, {
            width: 30,
            align: 'right'
            });
            doc.text(formatCurrency(item.precio_total), 530, yDet, {
            width: 25,
            align: 'right'
            });
        } else {
            doc.text(formatCurrency(item.precio_total), 500, yDet, {
            width: 55,
            align: 'right'
            });
        }
        ...
        // =============== CUADRO DE TOTALES (tipo “TOTAL SOLICITADO”) ===========
        yDet += 15;
        if (yDet > 680) {
        doc.addPage();
        yDet = 60;
        ...
        const totalBoxW = 200;
        const totalBoxH = (factura.tipo_compra === 'LOCAL' && factura.igv > 0) ? 45 : 30;
        const totalBoxX = 355;
        const totalBoxY = yDet;

        doc
        .rect(totalBoxX, totalBoxY, totalBoxW, totalBoxH)
        .stroke();

        let yTot = totalBoxY + 6;

        doc
        .font('Helvetica-Bold')
        .fontSize(9)
        .text('SUBTOTAL:', totalBoxX + 10, yTot);
        doc
        .font('Helvetica')
        .text(
            `${factura.moneda_simbolo || 'S/'} ${formatCurrency(factura.subtotal)}`,
            totalBoxX + 90,
            yTot,
            { width: 90, align: 'right' }
        );

        if (factura.tipo_compra === 'LOCAL' && factura.igv > 0) {
        yTot += 12;
        doc
            .font('Helvetica-Bold')
            .text('IGV (18%):', totalBoxX + 10, yTot);
        doc
            .font('Helvetica')
            .text(
            `${factura.moneda_simbolo || 'S/'} ${formatCurrency(factura.igv)}`,
            totalBoxX + 90,
            yTot,
            { width: 90, align: 'right' }
            );
        ...
        .font('Helvetica-Bold')
        .fontSize(10)
        .text('TOTAL:', totalBoxX + 10, yTot);
        doc
        .font('Helvetica-Bold')
        .text(
            `${factura.moneda_simbolo || 'S/'} ${formatCurrency(factura.total)}`,
            totalBoxX + 90,
            yTot,
            { width: 90, align: 'right' }
        );
        ...
        .font('Helvetica-Bold')
        .fontSize(10)
        .text('INFORMACIÓN ADICIONAL', 40, yDet);
        ...
        .fontSize(9)
        .text(
            `Moneda: ${factura.moneda_nombre} (${factura.moneda_codigo})`,
            40,
            yDet
        )
        .text(
            `Forma de Pago: ${factura.forma_pago_nombre || '-'}`,
            240,
            yDet
        )
        .text(
            `Tipo de Cambio: S/ ${formatCurrency(factura.tipo_cambio)}`,
            430,
            yDet
        );

        yDet += 12;
        doc.text(`Fecha Vencimiento: ${formatDate(factura.fecha_vencimiento)}`, 40, yDet);
        yDet += 12;
        doc.text(`Fecha Registro: ${formatDate(factura.fecha_registro)}`, 40, yDet);

        if (factura.comentario) {
        yDet += 12;
        doc
            .font('Helvetica-Bold')
            .text('Comentarios:', 40, yDet);
        yDet += 10;
        doc
            .font('Helvetica')
            .text(factura.comentario, 40, yDet, { width: 515 });
        }
